...
- **Триггер**: Событие `STABLE_STUDY` от Orthanc
- **Логика**: Ждёт, пока всё исследование (все серии) будет получено
- **Преимущества**: 
  - Маршрутизация полного исследования одной операцией
  - Все серии приходят вместе к получателю
  - Меньше сетевых операций
- **Недостатки**: Задержка до получения всех серий

### Series Mode
- **Триггер**: Событие `STABLE_SERIES` от Orthanc  
- **Логика**: Каждая серия маршрутизируется сразу при стабилизации
- **Преимущества**:
  - **Быстрее**: данные уходят сразу, не ждут других серий
  - Подходит для срочных исследований
  - Параллельная обработка больших исследований
- **Недостатки**: Может привести к множественным C-MOVE для одного исследования

**Переключение**: Через переменную окружения `ORTHANC_STABLE_MODE=series` или в Web UI → Configuration.
...
|------------|----------|----------------------|
| `ORTHANC_STABLE_MODE` | Режим маршрутизации: `study` (полное исследование) или `series` (по сериям) | `study` |
...
| `ORTHANC_MAX_DEFERRED` | Максимальное количество отложенных задач | `1000` |
| `ORTHANC_RETRY_MAX_ATTEMPTS` | Максимальное количество повторных попыток | `10` |

### Отложенные задачи (Deferred Tasks)

Отложенные задачи создаются когда destination недоступен. По умолчанию (`ORTHANC_DEFERRED_AUTO_REMOVE=true`) задачи удаляются после исчерпания всех попыток. 

Если установить `ORTHANC_DEFERRED_AUTO_REMOVE=false`, задачи с исчерпанными попытками сохраняются в SQLite базе данных с статусом `failed`. Это позволяет:
- Анализировать причины ошибок
- Видеть историю неудачных маршрутизаций
- Вручную обрабатывать накопившиеся задачи

Управлять опцией можно через Web UI → Configuration → «Автоудаление отложенных задач».
